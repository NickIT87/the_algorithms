...
def count_conflicts(graph, colors):
    """Count the number of conflicts in the graph for the given coloring."""
    conflicts = 0
    for node in graph.nodes():
        for neighbor in graph.neighbors(node):
            if colors[node - 1] == colors[neighbor - 1]:
                conflicts += 1
    return conflicts // 2  # Each conflict is counted twice (once for each direction)
